...
long amt_free[100];
//amt_free[0]=-1.0;
//memset(amt_free, 0, sizeof(amt_free));	//sets array to 0
int count1 = 0;
/* End of Add */
...
static void *slob_page_alloc(struct slob_page *sp, size_t size, int align)
{
	...
	for (prev = NULL, cur = sp->free; ; prev = cur, cur = slob_next(cur)) {
		...
		if (avail >= units + delta) { // room enough?
			int temp_delta = (avail - (units+delta));
			if( temp_delta < best_fit_delta){
				best_fit_delta = temp_delta;
				best_fit_cur = cur;
				best_fit_prev = prev;
			}
		}
		...
		if (slob_last(cur)){
			if(best_fit_cur == NULL){return NULL;}
